...
import Neo4jGraphPanel from './components/analyzer/Neo4jGraphPanel.vue'
...
const menuBreadcrumbs: Record<ConsoleSection, BreadcrumbItem[]> = {
  ...
  graph: [{ label: '图谱关系', section: 'graph' }],
    ...
    <ElContainer v-else-if="currentUser" class="shell">
      ...
      <ElContainer class="workspace">
        ...
        <Neo4jGraphPanel
          ...
          v-model:selected-chain-id="selectedGraphChainId"
          v-model:chain-filters="graphChainFilters"
          :busy="busy"
          :chains="graphChains"
          :selected-chain="selectedGraphChain"
          @apply-filters="applyGraphChainFilters"
          @refresh-all="loadGraphData"
          @select-chain="loadGraphChain"
        />
