...
for argument in "$@"; do
  case "$argument" in
    ...
    -h|--help)
      echo "Usage: bash scripts/start-on-site.sh [--skip-npm-ci] [--https]"
...
for command_name in docker od npm mvn java git codegraph curl nohup ps find grep seq tail tr; do
...
echo "[3/5] Starting PostgreSQL and Nginx components..."
component_mode="--components"
component_compose_file="compose.components.yaml"
if [[ -f "$ROOT_DIR/compose.offline.yaml" ]] \
  ...
  component_mode="--offline-components"
  component_compose_file="compose.offline.yaml"
  echo "Using locally imported offline component images."
fi
bash "$ROOT_DIR/scripts/start.sh" "$component_mode"

if [[ ! -f "$COMPONENT_ENV" ]]; then
  echo "Component environment file was not created: $COMPONENT_ENV" >&2
  exit 1
fi
...
if curl --fail --silent http://127.0.0.1:8080/actuator/health >/dev/null 2>&1; then
  echo "Port 8080 is already serving a backend not managed by $PID_FILE; refusing to start a duplicate." >&2
  exit 1
fi

echo "[4/5] Starting host backend..."
(
  set -a
  # shellcheck disable=SC1090
  ...
  set +a
  exec nohup java -Xms512m -Xmx2g -jar "$backend_jar"
...
echo "Analyzer Coder is ready at http://127.0.0.1:8088"
echo "Backend PID: $backend_pid"
echo "Backend log: $BACKEND_LOG"
echo "Component logs: docker compose --env-file .env.components -f $component_compose_file logs -f"
