...
test('full package pins exported image names into the environment template and records image identities', t => {
  ...
  assert.match(env, /POSTGRES_DATA_SOURCE=\/opt\/sunhuanshi\/analyzer-coder\/postgres/);
  assert.match(env, /POSTGRES_PORT=18080/);
  assert.match(env, /APP_HTTP_PORT=18081/);
  assert.doesNotMatch(env, /POSTGRES_STORAGE_TYPE|POSTGRES_VOLUME/);
  const compose = fs.readFileSync(path.join(f.output, 'components/compose.yaml'), 'utf8');
  assert.match(compose, /type: bind/);
  assert.match(compose, /source: \$\{POSTGRES_DATA_SOURCE:\?Set an absolute host path in components\/.env\}/);
  assert.match(compose, /target: \/var\/lib\/postgresql\/data/);
  assert.doesNotMatch(compose, /^name:|pull_policy:|create_host_path:|^volumes:\s*$/m);
  const nginx = fs.readFileSync(path.join(f.output, 'components/nginx/nginx.conf'), 'utf8');
  ...
  assert.match(backendConfig, /port: 18082/);
