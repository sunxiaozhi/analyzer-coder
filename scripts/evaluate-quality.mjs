...
function loadGold() {
  ...
  for (const [name, definition] of Object.entries(manifest.datasets)) {
    ...
    if (cases.length !== definition.expectedCount) fail(`${name} 应为 ${definition.expectedCount} 条，实际 ${cases.length} 条`);
    groups[name] = cases;
    for (const item of cases) {
      ...
      if (name === 'retrieval') {
        requireArray(item.expectedPaths, item.id, 'expectedPaths', true).forEach(file => assertFile(file, item.id));
      } else if (name === 'qa') {
        requireArray(item.evidence, item.id, 'evidence', true).forEach(entry => assertFile(entry.path, item.id));
        if (item.curation?.method !== 'HUMAN' || !item.curation?.reviewedAt) fail(`${item.id} 缺少人工证据审阅信息`);
      } else {
        fail(`不支持的数据集：${name}`);
      }
    }
  ...
  for (const type of manifest.repositoryTypes) {
    if (!Object.values(groups).flat().some(item => item.repositoryType === type)) fail(`数据集未覆盖仓库类型 ${type}`);
  }
  return groups;
}
...
function evaluate(gold, result) {
  ...
  for (const item of gold.qa) {
    ...
    for (const claim of requireArray(observed.claims, item.id, 'claims', true)) {
  ...
  return { datasetVersion: manifest.datasetVersion, runId: result.runId, measuredAt: new Date().toISOString(), metrics, thresholds, checks, passed: checks.every(item => item.passed) };
}
...
try {
  ...
  if (process.argv.includes('--validate')) {
    console.log(JSON.stringify({ valid: true, scoreable: true, datasetVersion: manifest.datasetVersion, counts: Object.fromEntries(Object.entries(gold).map(([key, value]) => [key, value.length])) }, null, 2));
