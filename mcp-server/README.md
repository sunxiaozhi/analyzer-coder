...
3. 联合检索需要已准备的代码快照。查询 Git 分支图谱还需要准备目标分支并发布 CodeGraph 产物；可在“项目管理”的分支列表操作，在 `list_codegraph_scopes` 中确认 `codegraphReady: true`。构建与准备不由 MCP 读取工具触发。

ZIP 单版本项目继续使用 `search_project` 的默认快照检索，不出现在分支图谱发现列表中。
...
1. 调用 `list_codegraph_scopes`，可传 `{ "page": 1, "pageSize": 20 }`。它只返回当前令牌可读取的 Git 项目和受管分支，每个分支包含名称、状态、`snapshotId`、`commitSha` 与 `codegraphReady`。只有 `codegraphReady: true` 的分支可做图谱查询。
...
3. `resolve_project_context` 返回 `contextId`、`snapshotId`、`commitSha`、分支名和过期时间。图谱工具返回的 `context` 字段也包含这些坐标，另有 `artifactId`、`cliVersion` 和 `result`。后续调用传 `repositoryId + contextId`，例如：
...
上例可用于 `codegraph_explore`。`contextId` 绑定账户和创建时的快照，有效期一小时；它不会随分支更新自动切换。过期后重新解析分支，并对照返回的 `commitSha`、`snapshotId`，确认是否切换到新版本。网页“分支工作区”的“复制 MCP 参数”也可取得项目和分支 ID。

## 工具目录

所有图谱工具都必须传 `repositoryId`，并且在 `branchId`、`contextId` 中至少传一个。不能传服务器文件路径、任意快照 ID 或图谱产物路径。下表列出额外参数；可选参数省略时使用服务端默认值。

| 工具 | 作用 | 额外参数 |
| --- | --- | --- |
| `list_codegraph_scopes` | 发现授权项目与分支 | 可选 `page`（默认 1）、`pageSize`（默认 20，最多 50） |
| `resolve_project_context` | 固定已准备分支快照 | `repositoryId`，以及 `branchId` 或 `contextId` |
...
只传 `repositoryId` 的 `search_project` 保持原有行为，读取项目默认快照；传入分支参数时返回 `{ "context": ..., "result": ... }`。图谱工具只读取所选分支的已发布受管产物，不执行测试、不构建图谱、不修改代码或知识。`codegraph_affected` 返回的是关联线索，并非实际运行的测试结果。
...
| `BRANCH_NOT_READY` | 所选分支没有已发布快照；在项目管理准备该分支。 |
| `CODEGRAPH_ARTIFACT_NOT_AVAILABLE` | 分支有快照但尚无已发布 CodeGraph 产物；构建图谱并确认 `codegraphReady`。 |
