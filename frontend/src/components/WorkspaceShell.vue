...
const navItems = computed(() => [
  ...
  { to: '/search', label: '代码检索', icon: Search }, { to: '/ask', label: '代码问答', icon: BookOpen },
  { to: '/graph', label: '调用图谱', icon: Boxes }, { to: '/knowledge', label: '知识卡片', icon: BookOpen },
...
<template><div class="app-shell">
  ...
  <main class="workspace"><header class="topbar"><span class="repository-label">当前仓库</span><el-select v-model="repositoryStore.selectedRepositoryId" class="global-repository-switcher" placeholder="请选择仓库" clearable><el-option v-for="repository in repositoryStore.repositories" :key="repository.id" :label="repository.name" :value="repository.id" /></el-select><div class="topbar-spacer" /><span class="context-chip">{{ auth.account?.displayName }} · {{ auth.isAdmin ? '管理员' : '普通用户' }}</span><el-button link title="退出登录" @click="logout"><LogOut :size="16" /></el-button></header>
    <div class="page-frame"><div class="page-context"><el-breadcrumb separator="/"><el-breadcrumb-item :to="{ path: '/repositories' }">代码知识平台</el-breadcrumb-item><el-breadcrumb-item>{{ pageTitle }}</el-breadcrumb-item></el-breadcrumb></div><div class="page-masthead"><div><h1>{{ pageTitle }}</h1><p>数据来自当前授权范围内的真实仓库、索引与知识数据。</p></div></div><div class="route-view"><RouterView /></div></div>
  </main>
</div></template>
