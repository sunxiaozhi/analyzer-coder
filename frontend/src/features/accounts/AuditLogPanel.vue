...
const actor=shallowRef(''),target=shallowRef(''),eventType=shallowRef(''),result=shallowRef(''),range=shallowRef<[Date,Date]|null>(null),page=shallowRef(1),pageSize=shallowRef(20);
...
<template>
  <div class="audit-panel">
    ...
    <div class="audit-pagination"><span>共 {{filteredRows.length}} 条</span><el-pagination v-model:current-page="page" v-model:page-size="pageSize" :page-sizes="[20,50,100]" layout="sizes, prev, pager, next" :total="filteredRows.length"/></div>
  </div>
</template>
