...
const branch = (id: string): RepositoryBranch => ({ id, name: id, snapshotId: 's-' + id, commitSha: 'abc', status: 'READY', error: null, generation: 1, trackingStatus: 'ACTIVE', archivedAt: null });
beforeEach(() => {
  vi.resetAllMocks();
  ...
  vi.mocked(branchesApi.context).mockResolvedValue({ contextId: 'ctx-release', repositoryId: 'p', branchId: 'release', branchName: 'release', snapshotId: 's-release', commitSha: 'abc', expiresAt: '' });
});
