...
const currentStatus=computed(()=>props.status?.contentVersion===props.context?.contentVersion?props.status:undefined);
const currentJobs = computed(() => {
  const running = props.jobs.filter(job => ['QUEUED', 'RUNNING'].includes(job.status));
  return running.length ? running : props.jobs.filter(job => job.status === 'FAILED' && job.contentVersion === props.branch.contentVersion);
});
...
const syncLabel=computed(()=>props.branch.status==='BUILDING'?'同步中':props.branch.status==='FAILED'?'上次同步失败':props.branch.contentVersion?'已同步':'未同步');
const labels:Record<string,string>={SYNC:'同步代码',CONTENT:'内容索引',GRAPH:'代码图谱',VECTORS:'向量索引',PREPARE:'一键准备'};
const stages:Record<string,string>={QUEUED:'等待执行',RESOLVING:'确认目标版本',SYNC:'同步代码',PUBLISHING:'发布分支代码',INDEXING:'构建内容索引',GRAPH:'构建代码图谱',EMBEDDING:'构建向量',COMPLETED:'已完成',FAILED:'失败'};
...
<template>
  <section class="branch-index-panel" aria-label="分支代码与索引">
    <header class="contentVersion-strip">
      ...
      <small>内容版本 {{ context?.contentVersion.slice(0,8) ?? branch.contentVersion?.slice(0,8) ?? '—' }}</small>
      <el-button v-if="branch.contentVersion" link type="primary" @click="emit('refresh')">刷新已准备版本</el-button>
    </header>
    <div class="index-states">
      <div><span>代码同步</span><strong>{{ syncLabel }}</strong><small>{{ syncedAtLabel }}</small></div>
      <div :data-ready="contentReady"><span>内容索引</span><strong>{{ contentReady?'已就绪':'待构建' }}</strong><small>当前内容版本的关键词与符号</small></div>
      <div :data-ready="graphReady"><span>代码图谱</span><strong>{{ graphReady?'已就绪':'待构建' }}</strong><small>当前内容版本的节点与关系</small></div>
    ...
    <div v-if="currentJobs.length" class="branch-task-list" aria-label="分支任务">
      <div v-for="job in currentJobs" :key="job.id" class="branch-task">
        ...
        <small>{{ job.contentVersion ? '内容版本 '+job.contentVersion.slice(0,8) : '等待锁定提交' }}{{ job.contentVersion && job.contentVersion!==branch.contentVersion?' · 历史版本任务':'' }}</small>
...
.contentVersion-strip{display:flex;align-items:center;flex-wrap:wrap;gap:12px;padding:14px 16px;background:#eff6ff;border-left:3px solid #2563eb}
.contentVersion-strip>div{display:grid;gap:4px}.scope-label{font-size:11px;color:#68778a}.contentVersion-strip code{font:13px Consolas,monospace}.contentVersion-strip small{font-size:12px;color:#68778a}
