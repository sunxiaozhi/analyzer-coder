...
const kinds: Record<string, string> = { SYNC: '同步代码', CONTENT: '内容索引', GRAPH: '代码图谱', VECTORS: '向量索引', PREPARE: '一键准备', SNAPSHOT: '代码准备' };
...
<template>
  <section class="branch-history" aria-label="当前分支任务记录">
    ...
    <div class="history-scroll"><table><thead><tr><th>任务</th><th>状态</th><th>执行阶段</th><th>代码快照</th></tr></thead><tbody>
      <template v-for="job in jobs" :key="job.id"><tr><td>{{ kinds[job.kind] }}</td><td>{{ states[job.status] }}</td><td>{{ stages[job.stage] ?? job.stage }}</td><td><code>{{ job.snapshotId?.slice(0, 8) || '等待锁定' }}</code></td></tr><tr v-if="job.error" class="history-error"><td colspan="4">{{ job.error }}</td></tr></template>
