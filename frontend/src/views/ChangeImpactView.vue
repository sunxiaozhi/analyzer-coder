...
<template>
  <section class="impact-page">
    <header class="impact-header">
      <div>
        <span class="eyebrow">变更影响工作台</span>
        <h1>先确认影响，再开始修改</h1>
        <p>结果来自当前仓库快照的混合检索与一跳模块依赖。它提供调查范围，不替代人工确认。</p>
      </div>
      ...
      <section class="result-main">
        ...
        <section class="candidate-section">
          ...
          <div v-if="analysis.candidates.length" class="candidate-list">
            <article v-for="item in analysis.candidates" :key="candidateKey(item)" :data-state="candidateState(item)">
