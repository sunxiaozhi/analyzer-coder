...
V1 中的仓库级当前版本字段仍供接入/迁移阶段写入；V7 触发器只把这条入口数据汇入默认分支。正常产品读写均以 `repository_branches` 为准。
...
| 迁移 | 作用 |
| --- | --- |
| V1 | 项目、索引、知识、问答、图谱等基础表 |
| V2 | 账号、权限、审计与系统配置 |
| V3 | 建立分支唯一边界；代码片段和知识直接绑定分支；阅读上下文与分支验证 |
| V4 | 分支图谱任务 |
| V5 | 分支准备任务 |
| V6 | 项目草稿、来源凭据与治理 |
| V7 | 分支生命周期、Markdown 来源分支化、问答证据分支化 |
| V8 | 分阶段代码操作、同步/索引就绪字段 |
| V9 | 项目总览与运行指标补充 |
