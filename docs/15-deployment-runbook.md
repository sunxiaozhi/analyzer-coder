...
数据库固定使用 bind，多个实例应使用不同的数据目录和端口。本次单一 V1 基线与旧版 Flyway 历史不兼容；升级前停止后端并备份需要保留的数据，然后删除并重建应用数据库。可继续使用原 `POSTGRES_DATA_SOURCE` 数据目录，但不能在旧库上直接启动新版后端。
...
- 空库启动时 Flyway 自动执行合并后的 V1。旧库需重新建库，不能只删除 `flyway_schema_history` 来绕过校验。
