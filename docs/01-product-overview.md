...
| 生成的答案缺少可核对来源 | 结果与问答都保留仓库、分支、内容版本、提交、知识修订标识 |

设计原则（可从实现中直接观察到）：

1. **一切结论绑定版本**：代码结果与知识结果都携带内容版本身份，不把不同分支或不同提交的材料混在一起。
2. **降级不伪装**：向量模型、CodeGraph 属于增强能力，未配置或不可用时保留关键词检索路径，并明确标注当前范围与降级原因。
3. **只读优先**：平台浏览与检索受管内容版本，不在原仓库中编辑或提交代码。
...
```
接入项目 → 准备分支内容版本 → 建立内容索引 → 维护知识 → 联合检索 → 回到来源
                                    ↘ 可选：向量索引 / 代码图谱 / 证据问答 / MCP
```

1. **接入项目**：在"项目管理"接入本地 Git、远程 Git/GitLab 或 ZIP 来源，得到项目（`repositoryId`）与受管内容版本。
2. **准备分支工作区**：所有项目按分支同步并“一键准备”（同步 → 内容索引 → 代码图谱）；ZIP 项目映射为固定逻辑分支 `WORKSPACE`。
...
阅读范围由**阅读上下文**固定：项目 + 分支 + 内容版本 + 提交，通过 `X-Branch-Context` 请求头传递，`contextId` 有效期 1 小时并绑定账号（`BranchReadContext`、`BranchRequestContext`、`RepositoryBranchService`）。
...
| 分支、内容版本与准备 | [04-branch-contentVersion-and-preparation.md](04-branch-contentVersion-and-preparation.md) | 分支跟踪与归档、内容版本发布与复用、准备任务与阶段、阅读上下文、内容版本保留与清理 |
| 代码索引与向量索引 | [05-code-index-and-vector.md](05-code-index-and-vector.md) | 片段切分与符号抽取、索引任务与增量、向量模型与覆盖、重建与复用 |
| 联合检索 | [06-unified-retrieval.md](06-unified-retrieval.md) | 多通道召回与排序、结果字段与来源、内容版本范围、降级诊断 |
...
| 内容版本（contentVersion） | 某一提交的不可变受管内容副本，标识 `contentVersion`，含 `commitSha` 与内容路径 |
| 提交（commitSha） | 内容版本对应的 Git 提交标识 |
| 内容令牌 | 分支当前发布代次的内部 UUID，用于并发与派生产物隔离，不是可管理的历史版本 |
| 阅读上下文 | 由项目 + 分支 + 内容版本 + 提交固定的一次阅读范围，`contextId` 有效 1 小时 |
...
| 准备（preparation） | 把某分支的提交发布为内容版本并建立索引的流程 |

## 6 版本与证据模型

这是全系统最重要的约束，跨所有领域成立：

1. **代码结果与知识结果都保留内容版本身份**，历史记录保留原引用内容版本，不随当前版本漂移。
2. **检索与代码读取限定当前阅读上下文**；未准备的分支不借用默认分支或其他分支产物。
