...
| --- | --- | --- |
| MCP 接入 | 按 mcp-server/README.md 配 API、Session、CSRF，启动 stdio 服务 | Agent → MCP → 平台 HTTP 接口与权限校验 |
...
CI 不替你运行测试、收集审批或配置流水线；报告由外部提供，未报告不等于通过。当前接入使用会话凭据，专用服务令牌尚未形成完整能力。
