...
- `LOCAL_HASH`：本地确定性 64 维字符哈希投影，只提供字符相似度，不具备语义理解；
- OpenAI-compatible `/embeddings`：支持 1–4096 维，检测必须确认返回长度等于备案维度。

向量记录保存模型标识、维度和检索能力（`CHARACTER_HASH` 或 `SEMANTIC_EMBEDDING`）。切换模型后不能把旧模型向量冒充为新模型结果；系统按当前实现触发重建/按需刷新。
...
模型密钥单独加密/脱敏存储，列表接口不回显明文。当前支持单仓多轮上下文与历史恢复；未实现模型流式回答、停止生成或断线续传。
