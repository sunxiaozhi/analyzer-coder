...
1. 模型备案已启用且连接检测通过；
2. 当前生效模型存在；
3. “允许发送检索命中的代码片段”策略允许外发。

任一门禁不满足时，返回本地证据型回答并标注拦截/降级信息，不应伪装成模型回答。前端刷新只读取配置，不产生“策略已更新”成功提示；成功提示仅在用户提交保存后出现。

模型密钥单独加密/脱敏存储，列表接口不回显明文。当前未实现模型流式回答、停止生成或历史多轮会话 UI，即使数据库保留了相关兼容表也不能视为已交付。
