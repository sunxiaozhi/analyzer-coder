...
查询时生成的展示对象，可以通过 `SearchResultSnapshot` 短期保存，也可以在复用时通过稳定源对象字段重建。关键字段：
...
| 系统配置 | 配置工作区、状态存储、默认 Qdrant 向量库、Neo4j 图数据库、embedding 服务、对象存储和路径策略，查看健康状态 |
...
核心可用版必须使用 Neo4j 保存项目内基础图关系，包括 CodeFact、接口、SQL、调用关系、Evidence、KnowledgeAsset、ContextPackage 和 ChangeTask 的关系。核心版只要求后端提供基础邻居查询、有限深度路径查询和裁剪后的 JSON 节点边数据，并在代码事实、接口关系或分析报告中查看基础关系。Cytoscape.js 交互式图谱、复杂路径探索、跨项目图谱、子图工作台和图节点直接加入上下文包属于增强版。
...
| SystemConfigService | 工作区、状态存储、默认 Qdrant、Neo4j、embedding、对象存储和路径策略配置 |
...
代码分析完成后还必须执行证据有效性检查：对比当前 RepositorySnapshot 与已绑定 Evidence 的文件路径、符号、行区间和签名。无法定位的 Evidence 生成治理项或 ReviewTask，不直接改变 KnowledgeAsset 状态。
...
4. 系统把基础代码关系、证据关系和知识关系写入 Neo4j，并支持项目内基础关系查询。
5. 系统构建当前项目 Qdrant 索引，支持按来源和可信状态检索。
6. 用户基于可信检索结果生成 Markdown 或纯文本上下文包，并记录引用清单和导出审计。
...
- 检索结果收藏、检索组合查询增强、交互式图谱页面、复杂路径探索和子图导出。
