...
治理运营负责把知识缺失、证据失效、索引失败、分析失败、低命中查询、上下文导出和影响分析命中回流为可处理的治理项或复审任务，帮助项目负责人维护知识可信度和数据新鲜度。
...
治理项。用于承接系统发现的问题、风险和待处理线索。关键字段：

- `id`
- `projectId`
- `sourceType`: `low_hit | missing_evidence | invalid_evidence | overdue_review | task_failed | change_hit | api_gap`
- `sourceId`
- `title`
- `summary`
- `severity`: `low | medium | high`
- `status`: `open | converted | resolved | ignored`
- `ownerUserId`
- `createdAt`
- `updatedAt`
...
- 治理项可以独立关闭，也可以转为复审任务。
- 转为复审任务后，治理项状态变为 `converted`，并记录对应复审任务 ID。
- 忽略治理项必须记录原因和审计事件。
...
```text
打开复审任务
  -> 查看触发原因、关联 KnowledgeAsset、Evidence、ChangeAnalysisTask 和图路径
  -> 项目负责人更新知识或证据，或确认无需处理
  -> resolved/ignored
  -> 回写 KnowledgeAsset/Evidence 状态
  -> 更新 Neo4j 图关系
  -> 标记 Qdrant 索引记录过期或触发索引任务
  -> 写入 AuditEvent
```
...
- 复审任务处理后必须写审计，并根据处理结果触发索引和图关系更新。
...
- 项目负责人可以看到知识状态统计、缺证据知识、待复审知识、复审任务、低命中查询、索引失败和分析失败。
- 项目负责人可以处理或忽略复审任务，处理和忽略都记录审计。
- 复审任务处理后会回写 KnowledgeAsset 或 Evidence 状态，并触发索引和图关系更新。
