...
- `fresh`：已完成索引，内容与源对象一致。
- `stale`：源对象发生变化，索引记录已过期，仍可作为旧结果展示但必须提示新鲜度风险。
- `rebuilding`：正在重建。
- `failed`：最近一次构建失败，保留失败原因和上一份可用索引。
...
- 快照保存的是当次结果清单和源对象定位信息，不替代源对象本身。
- 复用快照结果时，后端必须重新校验项目权限、源对象有效性和源对象状态。
- 源对象已失效时，不允许静默复用旧文本，只能提示失效并引导重新检索。
...
用户发起检索
  ...
  -> 后端使用 sourceType、sourceId、chunkId 或 resultSnapshotId 重新校验项目权限和源对象有效性
...
- SearchResultSnapshot 可以保留当次排序和展示状态，但不能作为唯一证据来源。
