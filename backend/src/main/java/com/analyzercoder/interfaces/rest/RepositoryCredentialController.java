...
public class RepositoryCredentialController {
    ...
    public RepositoryCredentialController(RepositoryCredentialService service) { this.service = service; }
    ...
    @PutMapping("/{id}")
    public CredentialView update(@PathVariable UUID id, @RequestBody CredentialInput input,
        HttpServletRequest request) {
        return service.update(SecurityContext.account(request), id, input, request.getRemoteAddr());
    }

    @PostMapping("/{id}/validate")
    public CredentialView validate(@PathVariable UUID id, @RequestBody ValidationInput input,
        HttpServletRequest request) {
        return service.validate(SecurityContext.account(request), id, input.repositoryUrl(), request.getRemoteAddr());
    }

    @PostMapping("/{id}/enable")
    public CredentialView enable(@PathVariable UUID id, HttpServletRequest request) {
        return service.setEnabled(SecurityContext.account(request), id, true, request.getRemoteAddr());
    }

    @PostMapping("/{id}/disable")
    public CredentialView disable(@PathVariable UUID id, HttpServletRequest request) {
        return service.setEnabled(SecurityContext.account(request), id, false, request.getRemoteAddr());
    }
    ...
    public List<RepositoryCredentialService.BindingView> bindings(@PathVariable UUID id, HttpServletRequest request) {
