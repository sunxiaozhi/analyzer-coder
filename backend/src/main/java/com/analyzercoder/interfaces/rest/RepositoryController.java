...
public class RepositoryController {
    ...
    public List<RepositoryResponse> list(HttpServletRequest request) {
        var account = SecurityContext.account(request);
        Set<UUID> visible = accessControl.visibleRepositoryIds(account).stream().collect(Collectors.toSet());
    ...
    public record UpdateRepositoryRequest(@NotBlank @Size(max = 100) String name, @Size(max = 500) String description,
                                          String defaultBranch, long version) {
    }
    ...
    public record RepositoryResponse(
            ...
            String branch, String commit, String worktreeDigest,
            boolean dirty, UUID snapshotId, Instant snapshotCreatedAt, String codeGraphPath, boolean codeGraphDetected,
            Instant lastScannedAt, UUID ownerAccountId, String ownerDisplayName, String relationship,
        ...
        static RepositoryResponse from(CodeRepository repository, RepositoryAccess access, CodeGraphArtifactRow artifact, RepositoryEditingService.Metadata metadata) {
