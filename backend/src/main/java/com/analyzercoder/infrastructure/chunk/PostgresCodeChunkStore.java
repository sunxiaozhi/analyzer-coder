...
import com.analyzercoder.domain.chunk.*;
...
public class PostgresCodeChunkStore implements CodeChunkStore {
    ...
    @Override public long countByRepositoryId(CodeRepositoryId id){return mapper.count(id.value(),null);}
    @Override public long countSearchByRepositoryId(CodeRepositoryId id,String query){return mapper.count(id.value(),query);}
    @Override public void deleteByRepositoryId(CodeRepositoryId id){mapper.deleteByRepositoryId(id.value());}
    private static CodeChunkRow row(CodeChunk chunk){return new CodeChunkRow(chunk.id().value(),chunk.repositoryId().value(),chunk.snapshotId().value(),chunk.commitSha(),chunk.filePath(),chunk.symbolId(),chunk.symbolName(),chunk.symbolKind(),chunk.language(),chunk.chunkType().name(),chunk.startLine(),chunk.endLine(),chunk.content(),chunk.contentHash(),chunk.createdAt());}
    private static CodeChunk domain(CodeChunkRow row){if(row.snapshotId()==null)throw new IllegalStateException("Code chunk is missing snapshot_id");return new CodeChunk(CodeChunkId.of(row.id()),CodeRepositoryId.of(row.repositoryId()),RepositorySnapshotId.of(row.snapshotId()),row.commitSha(),row.filePath(),row.symbolId(),row.symbolName(),row.symbolKind(),row.language(),ChunkType.valueOf(row.chunkType()),row.startLine(),row.endLine(),row.content(),row.contentHash(),row.createdAt());}
}