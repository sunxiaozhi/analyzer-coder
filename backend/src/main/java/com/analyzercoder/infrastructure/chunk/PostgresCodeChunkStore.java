...
public class PostgresCodeChunkStore implements CodeChunkStore {
    ...
    private static CodeChunk domain(CodeChunkRow row){if(row.snapshotId()==null)throw new IllegalStateException("代码片段缺少内容版本标识");return new CodeChunk(CodeChunkId.of(row.id()),CodeRepositoryId.of(row.repositoryId()),RepositorySnapshotId.of(row.snapshotId()),row.commitSha(),row.filePath(),row.symbolId(),row.symbolName(),row.symbolKind(),row.language(),ChunkType.valueOf(row.chunkType()),row.startLine(),row.endLine(),row.content(),row.contentHash(),row.createdAt());}
}
