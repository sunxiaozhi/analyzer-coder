...
public class AuthService {
    ...
    public AuthService(AuthMapper mapper, PasswordHasher passwordHasher, RepositoryAccessMapper repositoryAccessMapper, @Value("${app.security.initial-admin-username:}") String initialAdminUsername, @Value("${app.security.initial-admin-password:}") String initialAdminPassword, @Value("${app.security.session-idle-minutes:30}") long idleMinutes, @Value("${app.security.session-max-hours:12}") long maxHours, @Value("${app.security.lock-minutes:15}") long lockMinutes) {
        this.mapper = mapper;
    ...
    private static AccountRow account(AuthAccountRow row) {
        return row == null ? null : new AccountRow(row.id(), row.username(), row.displayName(), row.passwordHash(), AccountRole.valueOf(row.accountRole()), row.enabled(), row.mustChangePassword(), row.failedAttempts(), row.lockedUntil(), row.temporaryPasswordExpiresAt(), row.lastLoginAt(), row.lastLoginIp(), row.lastRepositoryId(), row.createdAt(), row.updatedAt(), row.accountVersion());
    }

    private static AccountSummary summary(AccountSummaryRow row) {
        String status = !row.enabled() ? "DISABLED" : row.lockedUntil() != null && row.lockedUntil().isAfter(Instant.now()) ? "LOCKED" : row.mustChangePassword() ? "PASSWORD_CHANGE_REQUIRED" : "ENABLED";
        return new AccountSummary(row.id(), row.username(), row.displayName(), AccountRole.valueOf(row.accountRole()), status, row.permissionCount(), row.lastLoginAt(), row.lastLoginIp(), row.createdAt(), row.updatedAt(), row.accountVersion());
    }
    ...
    private static Instant instant(Map<String, Object> row, String key) {
        Object value = value(row, key);
        return value == null ? null : value instanceof Instant i ? i : ((java.sql.Timestamp) value).toInstant();
    }
    ...
    public void initializeFirstAdmin() {
        if (mapper.accountCount() > 0) return;
        ...
        mapper.insertAccount(id, initialAdminUsername.trim(), "系统管理员", passwordHasher.hash(initialAdminPassword), AccountRole.SUPER_ADMIN.name(), true, now.plus(Duration.ofHours(24)), now);
    ...
    public LoginResult login(String username, String password, String sourceIp) {
        ...
        if (account.temporaryPasswordExpiresAt() != null && account.mustChangePassword() && account.temporaryPasswordExpiresAt().isBefore(now))
        ...
        if (!passwordHasher.matches(password == null ? "" : password, account.passwordHash())) {
            ...
            throw new ApiSecurityException(401, failures >= 3 && failures < 5 ? "CAPTCHA_REQUIRED" : "INVALID_CREDENTIALS", INVALID_LOGIN);
        }
    ...
    public Optional<AuthenticatedSession> authenticate(String rawToken) {
        ...
        AuthenticatedAccount account = new AuthenticatedAccount(row.accountId(), row.username(), row.displayName(), AccountRole.valueOf(row.accountRole()), row.mustChangePassword(), row.lastLoginAt(), row.lastRepositoryId());
        return Optional.of(new AuthenticatedSession(hash, row.csrfToken(), account));
    }

    @Transactional
    public LoginResult changePassword(AuthenticatedSession session, String currentPassword, String newPassword, String sourceIp) {
    ...
    public void updateLastRepository(AuthenticatedAccount account, UUID repositoryId) {
        if (repositoryId != null) {
            RepositoryAccessRow access = account.isSuperAdmin() ? repositoryAccessMapper.findMetadata(repositoryId) : repositoryAccessMapper.findAccess(account.id(), repositoryId);
            if (access == null) throw new ApiSecurityException(403, "FORBIDDEN", "无权限访问该仓库");
        }
    ...
    public PageResult<AccountSummary> pageAccounts(String query, int pageNum, int pageSize) {
        ...
        return PageResult.fromPage(mapper.listAccountsFiltered(normalized)).map(AuthService::summary);
    }

    @Transactional
    public CreatedAccount createAccount(UUID actorId, String username, String displayName, AccountRole role, String supplied, String sourceIp) {
        validateUsername(username);
        validateDisplayName(displayName);
        String password = supplied == null || supplied.isBlank() ? generateTemporaryPassword() : supplied;
        passwordHasher.validate(username, password);
        UUID id = UUID.randomUUID();
        Instant now = Instant.now();
        mapper.insertAccount(id, username.trim(), displayName.trim(), passwordHasher.hash(password), role.name(), true, now.plus(Duration.ofHours(24)), now);
        audit(actorId, id, null, "ACCOUNT_CREATED", "SUCCESS", sourceIp);
        return new CreatedAccount(summary(id), password);
    }

    @Transactional
    public AccountSummary updateAccount(UUID actorId, UUID targetId, String displayName, AccountRole role, Boolean enabled, Long expectedVersion, String sourceIp) {
        ...
        if (current.role() == AccountRole.SUPER_ADMIN && current.enabled() && (nextRole != AccountRole.SUPER_ADMIN || !nextEnabled) && mapper.enabledAdminCount() <= 1)
        ...
        if (mapper.updateAccount(targetId, nextName, nextRole.name(), nextEnabled, version, Instant.now()) == 0)
            throw new ApiSecurityException(409, "ACCOUNT_VERSION_CONFLICT", "账号资料已被其他操作修改，请刷新后重试");
        if (!nextEnabled || nextRole != current.role()) mapper.deleteAccountSessions(targetId);
        String event = !nextEnabled ? "ACCOUNT_DISABLED" : !current.enabled() ? "ACCOUNT_ENABLED" : nextRole != current.role() ? "ACCOUNT_ROLE_CHANGED" : "ACCOUNT_UPDATED";
    ...
    public List<PermissionView> permissions(UUID accountId) {
        return mapper.permissions(accountId).stream().map(row -> new PermissionView(uuid(row, "repo_id"), string(row, "name"), RepositoryPermission.valueOf(string(row, "permission_level")))).toList();
    }

    @Transactional
    public void setPermission(UUID actorId, UUID accountId, UUID repositoryId, RepositoryPermission permission, String sourceIp) {
    ...
    public List<AuditView> auditEvents(int limit, int offset) {
        return mapper.audits(Math.min(Math.max(limit, 1), 200), Math.max(offset, 0)).stream().map(row -> new AuditView(uuid(row, "id"), string(row, "event_type"), string(row, "result"), uuid(row, "request_id"), string(row, "source_ip"), string(row, "actor_username"), string(row, "target_username"), string(row, "repository_name"), instant(row, "created_at"))).toList();
    }

    public void audit(UUID actorId, UUID targetId, UUID repositoryId, String eventType, String result, String sourceIp) {
        mapper.insertAudit(UUID.randomUUID(), actorId, targetId, repositoryId, eventType, result, UUID.randomUUID(), sourceIp, Instant.now());
    }

    private LoginResult issueSession(AccountRow account) {
        String raw = randomToken(32), csrf = randomToken(24);
        Instant now = Instant.now();
        mapper.insertSession(PasswordHasher.sha256(raw), account.id(), csrf, now, now.plus(absoluteTimeout));
        return new LoginResult(raw, csrf, account.authenticated());
    }

    private Optional<AccountRow> findByUsername(String username) {
        return username == null ? Optional.empty() : Optional.ofNullable(account(mapper.findByUsername(username)));
    }
    ...
    private AccountSummary summary(UUID id) {
        ...
        if (row == null) throw new IllegalArgumentException("账号不存在");
        return summary(row);
    }
    ...
    public record AuditView(UUID id, String eventType, String result, UUID requestId, String sourceIp,
                            String actorUsername, String targetUsername, String repositoryName, Instant createdAt) {
    }

    public record AccountSummary(UUID id, String username, String displayName, AccountRole role, String status,
                                 int repositoryPermissionCount, Instant lastLoginAt, String lastLoginIp,
                                 Instant createdAt, Instant updatedAt, long version) {
    }

    private record AccountRow(UUID id, String username, String displayName, String passwordHash, AccountRole role,
        ...
        AuthenticatedAccount authenticated() {
            return new AuthenticatedAccount(id, username, displayName, role, mustChangePassword, lastLoginAt, lastRepositoryId);
        }
    }
}
