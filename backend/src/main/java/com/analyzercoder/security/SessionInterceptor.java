...
/** 在请求进入控制器前校验会话、CSRF 与强制改密状态，并建立请求安全上下文。 */
@Component
public class SessionInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(
        ...
        AuthenticatedSession session =
                auth.authenticate(cookie(request))
                        .orElseThrow(
                                () ->
                                        new ApiSecurityException(
                                                401, "SESSION_EXPIRED", "登录状态已失效，请重新登录"));
        ...
        if (session.account().mustChangePassword()
                && !Set.of("/api/auth/change-password", "/api/auth/logout", "/api/auth/me")
                        .contains(path)) {
