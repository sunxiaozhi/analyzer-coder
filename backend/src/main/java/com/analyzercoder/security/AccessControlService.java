...
/** 集中执行账户角色和仓库成员权限判断，确保不同入口采用一致的授权规则。 */
@Service
public class AccessControlService {
    private final RepositoryAccessMapper mapper;

    public AccessControlService(RepositoryAccessMapper mapper) {
        this.mapper = mapper;
    }
    ...
    public boolean canAccess(
        ...
        if (account.id().equals(access.ownerAccountId())) {
            return true;
        }
        return access.permissionLevel() != null
                && RepositoryPermission.valueOf(access.permissionLevel()).includes(required);
    }
    ...
    public List<UUID> visibleRepositoryIds(AuthenticatedAccount account) {
        return account.isSuperAdmin()
                ? mapper.findVisibleRepositoryIdsForAdmin()
                : mapper.findVisibleRepositoryIds(account.id());
    }

    public RepositoryAccess describe(AuthenticatedAccount account, CodeRepositoryId repositoryId) {
        RepositoryAccessRow row =
        ...
        RepositoryPermission permission =
                owner || admin
                        ...
                        : RepositoryPermission.valueOf(row.permissionLevel());
        String relationship = admin ? "SUPER_ADMIN" : owner ? "OWNER" : permission.name();
        boolean maintain = permission.includes(RepositoryPermission.MAINTAIN),
                manage = permission.includes(RepositoryPermission.MANAGE);
        return new RepositoryAccess(
                row.ownerAccountId(),
                row.ownerDisplayName(),
                relationship,
                row.ownershipVersion(),
                row.repositoryStatus(),
                new RepositoryAccess.Capabilities(
                        true,
                        manage,
                        maintain,
                        maintain,
