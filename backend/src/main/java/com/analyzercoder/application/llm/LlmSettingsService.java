...
public class LlmSettingsService {
    ...
    public ProviderView latest() {
        ...
        if (row == null) return emptyProviderView();
    ...
    public CheckView startCheck(UUID actorId, ConnectivityCheckRequest request) {
        ProbeCandidate candidate = request.configId() == null
        ...
        if (existing != null) {
            ...
            if (existingRow != null && isNonTerminal(string(existingRow, "status"))) return checkView(existingRow);
        ...
        if (raced != null) {
            ...
            mapper.cancelCheck(checkId, stringJson(List.of(
                new StageView("SINGLE_FLIGHT_REUSED", "CANCELED", 0, "LLM_CHECK_REUSED")
            )));
    ...
    public CheckView cancelCheck(UUID actorId, UUID checkId) {
        ...
        mapper.cancelCheck(checkId, stringJson(List.of(
            new StageView("CANCELED", "CANCELED", 0, "LLM_CHECK_CANCELED")
        )));
    ...
    public ProviderView activate(UUID actorId, UUID configId, ActivationRequest request) {
        ...
        if (checkId == null || mapper.recentAvailableCheck(
            configId,
    ...
    public Optional<GenerationResult> generate(String prompt) {
        if (!mapper.externalModelEnabled()) return Optional.empty();
        if (prompt == null || prompt.length() > 24000 || containsSensitiveContent(prompt)) {
            return Optional.empty();
        }
        Map<String, Object> row = mapper.activeConfig();
        if (row == null || !"AVAILABLE".equals(string(row, "availability"))
            || !"CLOSED".equals(string(row, "breaker_state"))) return Optional.empty();
    ...
    private void runCheck(UUID checkId, ProbeCandidate candidate, String flightKey, CheckControl control) {
        ...
        try {
            ...
            OpenAiCompatibleClient.ProbeResult result = client.probe(
                candidate.spec(),
                candidate.apiKey(),
                deadline,
                control.canceled,
                event -> {
                    stages.add(new StageView(
                        event.stage(), event.status(), event.durationMs(), event.errorCode()
                    ));
                    mapper.updateCheckProgress(checkId, event.stage(), stringJson(stages));
                }
            );
            stages.add(new StageView("CLASSIFY_AND_PERSIST", "SUCCEEDED", 0, null));
            int completed = mapper.completeCheck(
                checkId,
                "SUCCEEDED",
                result.availability(),
            ...
            if ("LLM_CHECK_CANCELED".equals(exception.code()) || control.canceled.get()) {
                ...
                int completed = mapper.completeCheck(
                    ...
                    "FAILED",
                    "UNAVAILABLE",
                    stages.isEmpty() ? "VALIDATE_CONFIG" : stages.get(stages.size() - 1).stage(),
                    stringJson(stages),
                    exception.code(),
    ...
    private SecretMaterial resolveSecretForSave(
        ...
        if ("CLEAR".equals(action)) return new SecretMaterial(null, "none");
        if (!"REPLACE".equals(action)) {
            throw new ApiSecurityException(400, "LLM_CONFIG_INVALID", "secretAction 必须为 KEEP、REPLACE 或 CLEAR");
        }
    ...
    private ValidatedInput validate(ProviderInput input) {
        if (input == null) throw new ApiSecurityException(400, "LLM_CONFIG_INVALID", "模型配置不能为空");
    ...
    private ProviderView emptyProviderView() {
        ...
        return new ProviderView(
            null, 0, "", "OPENAI_COMPATIBLE", "", "", 5000, 60000, 2048, 0.2, true,
            false, null, "UNCONFIGURED", false, uuid(activation, "active_config_id"),
            number(activation, "activation_version", 0),
            null, null, null, null, "CLOSED", null, null
        );
    }
    ...
    private String readSecret(UUID secretId) {
        ...
        if (secret == null) throw new ApiSecurityException(409, "LLM_SECRET_MISSING", "模型密钥版本不存在");
    ...
    private Map<String, Object> requireConfig(UUID id) {
        ...
        if (row == null) throw new ApiSecurityException(404, "LLM_CONFIG_NOT_FOUND", "模型配置不存在");
        return row;
    }

    private Map<String, Object> requireCheck(UUID id) {
        Map<String, Object> row = mapper.check(id);
        if (row == null) throw new ApiSecurityException(404, "LLM_CHECK_NOT_FOUND", "连接检测不存在");
        return row;
    }
    ...
    private static String safeSummary(String value) {
        if (value == null || value.isBlank()) return "连接检测失败";
