...
public class RepositoryEditingService {
    ...
    public RepositoryEditingService(RepositoryMapper mapper, AccessControlService accessControl, AuthService authService) {
        this.mapper = mapper;
    ...
    @Transactional
    public void update(AuthenticatedAccount actor, UUID repositoryId, String name, String description,
                       String defaultBranch, long expectedVersion, String sourceIp) {
        accessControl.require(actor, CodeRepositoryId.of(repositoryId), RepositoryPermission.MANAGE);
        ...
        if (nextDescription.length() > 500) throw new IllegalArgumentException("仓库描述不能超过 500 个字符");
        String nextBranch = defaultBranch == null ? current.defaultBranch() : defaultBranch.trim();
        if (nextBranch != null && nextBranch.length() > 255) throw new IllegalArgumentException("默认分支不能超过 255 个字符");
        String normalizedName = nextName.toLowerCase(Locale.ROOT);
        if (mapper.countByOwnerAndNormalizedNameExcludingId(current.ownerAccountId(), normalizedName, repositoryId) > 0) {
            throw new ApiSecurityException(409, "REPOSITORY_NAME_CONFLICT", "该所有者名下已存在同名仓库");
        }
        if (mapper.updateEditableMetadata(repositoryId, nextName, normalizedName, nextDescription,
            nextBranch == null || nextBranch.isBlank() ? null : nextBranch, expectedVersion) != 1) {
            throw new ApiSecurityException(409, "REPOSITORY_VERSION_CONFLICT", "仓库资料已被其他操作修改，请刷新后重试");
        }
        authService.audit(actor.id(), null, repositoryId, "REPOSITORY_UPDATED", "SUCCESS", sourceIp);
    }
