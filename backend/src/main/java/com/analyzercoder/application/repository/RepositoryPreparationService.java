...
public class RepositoryPreparationService {
    ...
    static ProjectProfile profile(
        ...
        if (entryPoints.size() > 8) {
            entryPoints = new ArrayList<>(entryPoints.subList(0, 8));
        }
    ...
    private static List<ProfileCount> topCounts(Map<String, Long> values, int limit) {
        return values.entrySet().stream()
                ...
                .limit(limit)
                .map(entry -> new ProfileCount(entry.getKey(), entry.getValue()))
                .toList();
    }
    ...
    private static boolean isEntryPoint(String fileName) {
        ...
        return List.of("main", "app", "application", "index", "server", "cli", "bootstrap")
                        .contains(stem)
                || List.of(
                                "pom.xml",
                                "package.json",
    ...
    private static String normalizedLabel(String value, String fallback) {
        return value == null || value.isBlank() || "text".equalsIgnoreCase(value)
                ? fallback
                : value;
    }
    ...
    private static boolean graphJobFailed(IndexJob job) {
        return job != null
                && job.type() == IndexJobType.CODEGRAPH
    ...
    private static String jobDetail(IndexJob job, String fallback) {
        ...
        if (job.errorMessage() != null && !job.errorMessage().isBlank()) {
            return job.errorMessage();
        }
        return job.currentStep() == null || job.currentStep().isBlank()
                ? fallback
                : job.currentStep();
    }
