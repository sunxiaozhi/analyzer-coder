...
/** 编排仓库凭据相关应用流程，协调领域对象、权限校验与基础设施端口。 */
@Service
public class RepositoryCredentialService {
    ...
    @Transactional
    public CredentialView create(
            AuthenticatedAccount actor, CredentialInput input, String sourceIp) {
    ...
    public CredentialView update(
            AuthenticatedAccount actor, UUID id, CredentialInput input, String sourceIp) {
        ...
        CredentialSecretCipher.EncryptedSecret encrypted =
                replaceSecret ? cipher.encrypt(value.secret()) : null;
        mapper.update(
                id,
                value.type(),
                value.displayName(),
                ...
                encrypted == null ? null : encrypted.iv(),
                encrypted == null ? null : encrypted.digest(),
                encrypted == null ? null : encrypted.algorithm(),
                replaceSecret ? mask(value.secret()) : string(existing, "masked_value"),
                actor.id(),
                replaceSecret);
    ...
    public CredentialView validate(
        ...
        try {
            ...
            auth.audit(
                    actor.id(), null, null, "REPOSITORY_CREDENTIAL_VALIDATED", "SUCCESS", sourceIp);
        } catch (RuntimeException exception) {
            mapper.updateValidation(id, "INVALID", Instant.now(), safeError(exception));
            auth.audit(
                    actor.id(), null, null, "REPOSITORY_CREDENTIAL_VALIDATED", "DENIED", sourceIp);
            throw exception;
        }
        return view(requireOwned(actor, id));
    }
    ...
    public Resolved resolveInternal(UUID id, String repositoryUrl) {
        ...
        if (row == null) {
            throw new IllegalArgumentException("Git 凭据不存在");
        }
        return resolveRow(row, repositoryUrl);
    }
    ...
    private Resolved resolveRow(Map<String, Object> row, String repositoryUrl) {
        ...
        if (username == null || username.isBlank()) {
            username = "GITLAB_PAT".equals(string(row, "credential_type")) ? "oauth2" : "git";
        }
        return new Resolved(
                uuid(row, "id"),
    ...
    public Resolved resolveBound(
            AuthenticatedAccount actor, UUID repositoryId, String repositoryUrl) {
    ...
    public CredentialView setEnabled(
        ...
        auth.audit(
                ...
                null,
                null,
                enabled ? "REPOSITORY_CREDENTIAL_ENABLED" : "REPOSITORY_CREDENTIAL_DISABLED",
                "SUCCESS",
                sourceIp);
        return view(requireOwned(actor, id));
    }
    ...
    public List<BindingView> bindings(AuthenticatedAccount actor, UUID id) {
        requireOwned(actor, id);
        return mapper.bindings(id).stream()
                .map(
                        row ->
                                new BindingView(
    ...
    public void unbind(AuthenticatedAccount actor, UUID repositoryId) {
        ...
        if (bound != null) {
            requireOwned(actor, uuid(bound, "id"));
        }
    ...
    private static ValidatedInput validateInput(CredentialInput input, boolean requireSecret) {
        ...
        if (!TYPES.contains(type)) {
            throw new IllegalArgumentException("不支持的 Git 凭据类型");
        }
        String name = normalized(input.displayName());
        if (name.isBlank() || name.length() > 100) {
            throw new IllegalArgumentException("凭据名称长度必须为 1-100 个字符");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("Git 服务地址格式无效");
        }
        ...
        if (requireSecret && secret.length() < 8) {
            throw new IllegalArgumentException("访问令牌至少包含 8 个字符");
        }
    ...
    private static void requireMatchingServer(String configured, String target) {
        ...
        if (!"https".equalsIgnoreCase(actual.getScheme())
            ...
            throw new IllegalArgumentException("所选凭据与远程仓库主机不匹配");
        }
    }
    ...
    private CredentialView view(Map<String, Object> row) {
        return new CredentialView(
                uuid(row, "id"),
                string(row, "credential_type"),
                string(row, "display_name"),
                string(row, "server_url"),
                string(row, "username"),
                string(row, "masked_value"),
                string(row, "status"),
                instant(row, "last_validated_at"),
                uuid(row, "created_by"),
                instant(row, "created_at"),
    ...
    private static String safeError(RuntimeException exception) {
        ...
        return value == null ? "检测失败" : value.substring(0, Math.min(240, value.length()));
    }
    ...
    private static UUID uuid(Map<String, Object> row, String key) {
        Object v = value(row, key);
        return v == null ? null : v instanceof UUID id ? id : UUID.fromString(v.toString());
    }
    ...
    public record CredentialInput(
            String type, String displayName, String serverUrl, String username, String secret) {}
    ...
    public record BindingView(
            UUID repositoryId, String repositoryName, String usageType, Instant createdAt) {}
