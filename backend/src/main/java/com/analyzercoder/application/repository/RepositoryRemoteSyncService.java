...
/** 同步远程 Git 仓库并记录版本变化，凭据仅在受控的拉取过程内使用。 */
@Service
public class RepositoryRemoteSyncService {
    ...
    public RepositoryRemoteSyncService(
    ...
    public SyncResult sync(AuthenticatedAccount actor, CodeRepositoryId repositoryId) {
        ...
        if (scan.changed()) {
            jobId =
                    indexJobs
                            .start(new StartIndexCommand(repositoryId, IndexJobType.INCREMENTAL))
                            .id()
                            .value();
        }
    ...
    public record SyncResult(
            boolean changed,
            com.analyzercoder.domain.repository.CodeRepository repository,
