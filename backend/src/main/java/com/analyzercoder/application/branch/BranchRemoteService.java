...
public class BranchRemoteService {
    ...
    public BranchRemoteService(RepositoryMapper repositories,RepositoryCredentialService credentials,GitCredentialExecutor git,AccessControlService access) {
        this.repositories=repositories;this.credentials=credentials;this.git=git;this.access=access;
    }
    public List<GitCredentialExecutor.RemoteBranch> discover(AuthenticatedAccount actor,UUID repoId) {
    ...
    public String fetch(AuthenticatedAccount actor,CodeRepository repository,String branch) {
        ...
        return git.fetchBranch(repository.path(),url,branch,credential==null?null:credential.value());
    }
    private String url(UUID repoId) {
        String url=repositories.findRemoteUrl(repoId);
        if(url==null || url.isBlank()) throw new IllegalArgumentException("当前仓库未配置远程地址");
