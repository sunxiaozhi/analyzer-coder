...
public class AnswerCitationValidator {
    ...
    public Validation validate(String answer, int evidenceCount) {
        if (answer == null || answer.isBlank()) return Validation.invalid("模型没有返回回答");
        Matcher matcher = CITATION.matcher(answer);
        Set<Integer> cited = new LinkedHashSet<>();
        boolean invalidReference = false;
        while (matcher.find()) {
            int index = Integer.parseInt(matcher.group(1));
            if (index < 1 || index > evidenceCount) invalidReference = true;
            else cited.add(index);
        }
        if (invalidReference) return Validation.invalid("模型引用了不存在的证据");
        if (cited.isEmpty()) return Validation.invalid("模型回答缺少证据引用");
