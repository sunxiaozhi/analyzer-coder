...
public class MarkdownRenderingService {
    ...
    public MarkdownRenderingService() {
        ...
        renderer = HtmlRenderer.builder().extensions(extensions).escapeHtml(true).sanitizeUrls(true).build();
        policy = new HtmlPolicyBuilder()
            .allowElements("p", "br", "hr", "strong", "em", "del", "blockquote", "pre", "code",
                "ul", "ol", "li", "h1", "h2", "h3", "h4", "h5", "h6",
                "table", "thead", "tbody", "tr", "th", "td", "a", "img")
            .allowAttributes("href").matching(SAFE_URL).onElements("a")
            .allowAttributes("src").matching(SAFE_URL).onElements("img")
