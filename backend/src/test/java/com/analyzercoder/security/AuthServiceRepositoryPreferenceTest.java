...
class AuthServiceRepositoryPreferenceTest {
    ...
    void setUp() {
        ...
        service = new AuthService(
            authMapper, new PasswordHasher(), accessMapper, "", "", 30, 12, 15
        );
    ...
    void savesAccessibleRepository() {
        ...
        when(accessMapper.findAccess(account.id(), repositoryId)).thenReturn(
            new RepositoryAccessRow(repositoryId, account.id(), "普通账号", null, 1, "READY")
        );

        service.updateLastRepository(account, repositoryId);

        verify(authMapper).updateLastRepository(eq(account.id()), eq(repositoryId), any(Instant.class));
    }
    ...
    void rejectsRepositoryOutsideAccountScope() {
        ...
        assertThatThrownBy(() -> service.updateLastRepository(account, repositoryId))
            .isInstanceOfSatisfying(ApiSecurityException.class, exception -> {
                org.assertj.core.api.Assertions.assertThat(exception.status()).isEqualTo(403);
