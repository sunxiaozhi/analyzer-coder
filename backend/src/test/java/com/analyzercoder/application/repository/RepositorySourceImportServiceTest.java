...
class RepositorySourceImportServiceTest {
    ...
    void translatesMissingRemoteBranchWithoutLeakingEnglishOutput() {
        String message = RepositorySourceImportService.gitFailureMessage(
            List.of("clone", "--depth", "1", "--branch", "main", "https://example/repo.git", "target"),
    ...
    void translatesAuthenticationAndNetworkFailures() {
        assertThat(RepositorySourceImportService.gitFailureMessage(
            List.of("clone"), "fatal: Authentication failed"
        )).contains("身份验证失败");
        assertThat(RepositorySourceImportService.gitFailureMessage(
            List.of("clone"), "fatal: unable to access: Could not resolve host"
        )).contains("无法解析远程仓库域名");
        assertThat(RepositorySourceImportService.gitFailureMessage(
