...
class RepositoryPreparationServiceTest {
    ...
    void startsFullIndexWhenPublishedSnapshotHasNoChunks() {
        ...
        RepositoryPreparationService service = new RepositoryPreparationService(
            repositories, remoteSync, browser, vectors, graphArtifacts, indexJobs, jobStore, graphTasks
        );

        CodeRepository repository = repository();
        VectorIndexQueryService.Summary empty = new VectorIndexQueryService.Summary(
            repository.id().value(), repository.currentSnapshotId().value(), repository.currentCommit(),
            0, 0, 0, 0, 0, "local-hash-64", 64, null
        );
        ...
        when(browser.list(repository.id())).thenReturn(new RepositoryCodeBrowserService.SnapshotFiles(
            repository.currentSnapshotId().value().toString(), "main", repository.currentCommit(), List.of()
        ));

        RepositoryPreparationService.PreparationView result = service.prepare(
            mock(AuthenticatedAccount.class), repository.id()
        );

        ArgumentCaptor<StartIndexCommand> command = ArgumentCaptor.forClass(StartIndexCommand.class);
        ...
        assertThat(result.stages()).extracting(RepositoryPreparationService.PreparationStage::state)
    ...
    void buildsDeterministicProfileFromPublishedFiles() {
        ...
        VectorIndexQueryService.Summary summary = new VectorIndexQueryService.Summary(
            repositoryId, snapshotId, "abc", 12, 10, 2, 3, 3, "local-hash-64", 64, Instant.now()
        );
        List<RepositoryCodeBrowserService.FileEntry> files = List.of(
            new RepositoryCodeBrowserService.FileEntry("src/Main.java", "Main.java", "java", 100),
            new RepositoryCodeBrowserService.FileEntry("src/App.vue", "App.vue", "vue", 200),
            new RepositoryCodeBrowserService.FileEntry("backend/server.py", "server.py", "python", 300),
            new RepositoryCodeBrowserService.FileEntry("package.json", "package.json", "json", 50),
            new RepositoryCodeBrowserService.FileEntry("README.md", "README.md", "markdown", 25)
        );

        RepositoryPreparationService.ProjectProfile profile = RepositoryPreparationService.profile(files, summary, null);

        assertThat(profile.fileCount()).isEqualTo(5);
        assertThat(profile.totalBytes()).isEqualTo(675);
        assertThat(profile.modules()).extracting(RepositoryPreparationService.ProfileCount::name)
            .containsExactly("src", "backend");
        assertThat(profile.entryPoints()).containsExactly(
    ...
    private static CodeRepository repository() {
        ...
        return new CodeRepository(
            CodeRepositoryId.newId(), "sample", Path.of("sample"), RepositorySourceType.LOCAL_GIT,
            "main", "abc", "digest", false, RepositorySnapshotId.newId(), Path.of("snapshot"),
            Path.of("snapshot/.codegraph"), now, now, now, now
        );
    }
}
